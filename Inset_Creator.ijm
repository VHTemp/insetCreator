...
Dialog.addCheckbox("Adjust brightness:", false);
Dialog.addCheckbox("Perform color balance (only for RGB image):", false);
Dialog.addCheckbox("Rotate the image: ", false);
Dialog.addCheckbox("Crop the image:", false);
Dialog.addNumber("Number of insert: ", 1);
Dialog.addCheckbox("Check everytime that ROI was added in ROI Manager", false);
Dialog.addCheckbox("Do you want to save the ROI to make the inset?", false);
Dialog.show();
...
saveInsetROI = Dialog.getCheckbox();
...
function colorBalance() {
	...
	imgArray = newArray(title, "temp");
	
	for (img = 0; img < imgArray.length; img++) {
		selectImage(imgArray[img]);
		run("Select None");
	...
	if (mean > 150) {
		selectImage(title);
		setMinAndMax(0, mean+10);
	} else if(mean < 50) {
		setMinAndMax(0, 255);	
	}
	selectImage(title);
	run("Brightness/Contrast...");
	waitForUser("Check Brightness");
